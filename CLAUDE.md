...
| File | Description |
|------|-------------|
| `data_for_project/merged_profiles.csv` | **Main input** — 36,807 profiles, 22 columns (normalized text + user_folder + platform) |
| `data_for_project/combined_profiles.csv` | Raw profiles + user_folder + platform |
| `data_for_project/nomalized_profiles.csv` | Normalized text + cleaned URLs |
...
| `train_data/ground_truth_matrix.npz` | Sparse binary matrix 36,807×36,807 (ground truth) |
| `train_data/ground_truth_matrix_index.csv` | matrix_idx → userName, user_folder |

## Pipeline Stages (train_data/)
```
