...
DEFAULT_ALL_PROFILES = PROJECT_ROOT / "data" / "processed" / "all_profiles_cleaned.csv"
DEFAULT_PAIR_FILES = [
    PROJECT_ROOT / "data" / "processed" / "pairs_instagram_googleplus.csv",
    PROJECT_ROOT / "data" / "processed" / "pairs_twitter_googleplus.csv",
    PROJECT_ROOT / "data" / "processed" / "pairs_twitter_instagram.csv",
]
DEFAULT_LOCATION_MAPPING = PROJECT_ROOT / "data" / "processed" / "location_mapping.csv"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "data" / "processed" / "bigram_pipeline_results"
...
def main() -> None:
    ...
    parser.add_argument(
        "--all-profiles",
        default=str(DEFAULT_ALL_PROFILES),
        help="Master cleaned profile file",
    )
    parser.add_argument(
        "--pair-files",
        nargs="+",
        default=[str(path) for path in DEFAULT_PAIR_FILES],
        help="Pair files",
    )
    parser.add_argument(
        "--location-mapping-file",
        default=str(DEFAULT_LOCATION_MAPPING),
